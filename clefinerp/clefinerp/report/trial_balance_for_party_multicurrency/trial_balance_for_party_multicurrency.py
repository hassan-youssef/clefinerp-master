...
def get_data(filters):	
	...
	if not filters.get("account"):
		...
		suppliers = frappe.db.get_all('Supplier' ,'name')
		results = []
		for c in customers:		
			filters["party"] = [c.name]
			filters["presentation_currency"] = presentation_currency			
			results.append(get_results(filters))
		for s in suppliers:		
			filters["party"] = [s.name]
			filters["presentation_currency"] = presentation_currency			
			results.append(get_results(filters))			
	else:
		account_list = []	
		if filters.get("account"):   
			account_list.append(filters.get("account"))		
			filters["account"] = account_list
		parties = frappe.db.sql(f"""
		SELECT DISTINCT party
		FROM `tabGL Entry`
		WHERE company = '{filters.get("company")}' 
		AND is_cancelled = 0
		AND account = '{account_list[0]}'
	...
	if filters.get("account") or (filters.get("party") and len(filters["party"]) == 1):
		...
		if filters.get("account"):
			...
			if gle_currency:
				...
				account_currency = (None if filters.get("party_type") in ["Employee", "Student", "Shareholder", "Member" , None] else
